...
{
    header('Location: https://'.$_SERVER['HTTP_HOST'].'/login.php?redirect=/full.php'.(empty($_SERVER['QUERY_STRING']) ? "" : "&".$_SERVER['QUERY_STRING']), TRUE, 307);
    die();
}
?>
<!DOCTYPE html>
...
<script type="text/javascript">
$(function() {
    LoadPage( '<?php echo $PAGELOADER['id']; ?>', { } );
});

function LoadPage( pageName, pageOptions ) {
    var getParams;
    getParams = getUrlVars();
    switch( pageName ){
        case 'editor':
            urlparams = {};
            if( getParams['EmailTemplateID'] ) {
                urlparams.EmailTemplateID = getParams['EmailTemplateID'];
            }
            if( getParams['PaperTemplateID'] ) {
                urlparams.PaperTemplateID = getParams['PaperTemplateID'];
            }
            if( getParams['BulkEmailID'] ) {
                urlparams.BulkEmailID = getParams['BulkEmailID'];
            }
            LoadContent('fullContainer', '/load.php?do=editorHeader', { divid: 'hdrEditor', urlparams: urlparams, spinner: true } );
            LoadContent('fullContainer', '/load.php?do=ckeditor', { divid: 'ckEditorWrapper', urlparams: urlparams, spinner: true } );
            break;
    }
}
